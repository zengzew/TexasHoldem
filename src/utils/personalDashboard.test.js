...
describe('personal dashboard helpers', () => {
  ...
  it('builds ascending trend for all sessions by default', () => {
    ...
    expect(trend[11].sessionId).toBe('s12');
    expect(trend[0].ordinal).toBe(1);
    expect(trend[11].ordinal).toBe(12);
  });

  it('still supports an explicit recent-session limit', () => {
    const rows = Array.from({ length: 12 }, (_, index) => ({
      sessionId: `s${index + 1}`,
      createdAt: `2026-03-${String(index + 1).padStart(2, '0')}T12:00:00.000Z`,
      netResult: index + 1,
    ...
    const trend = buildPersonalTrend(rows, 10);
