...
- 房主可添加玩家与移除玩家（`room_players`）
- 刷新后自动恢复用户上次加入房间（本地存储按用户隔离）

3. 结算模块
- 录入买入与最终积分
- 支持负数买入冲正（用于误确认纠错）
- 累计总买入下限为 0（不会出现负值）
- 玩家确认按钮带 loading + in-flight 防抖（避免重复提交）
- 结算前校验：总净输赢为 0
- 产出转账建议并写入 `transfers`

4. 历史模块
- 历史记录分页 + 对局详情展开
- 历史积分榜（净盈利 / 场均盈利 / 金额 / 场均金额 / 盈利场次，详情含最大单局盈利、最大单局亏损）
- 积分榜支持弹出式日期范围筛选（默认当年 1 月 1 日到今天）
- 个人看板仅统计当前用户参与过的全部已结算对局，不使用日期筛选；展示与积分榜对齐的累计指标、总买入、最大单局盈利/亏损与盈亏趋势

## 3. 数据流（简）

1. 登录成功后加载：
- `profiles`（当前用户昵称）
- `sessions` / `room_players`（仅当前房间恢复所需）
- 懒加载策略：
  - 历史积分榜：切到“积分榜”Tab 后首次读取全量 `session_players`
  - 个人看板：切到“个人看板”Tab 后首次读取当前用户参与过的已结算 `session_players`
  - 历史记录：切到“历史记录”Tab 后首次读取当前用户参与过的对局
  - 玩家目录：房主进入“全部玩家”并聚焦搜索后首次读取 `profiles`
